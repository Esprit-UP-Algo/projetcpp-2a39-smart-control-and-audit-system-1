...
connect(ui->enovyer, SIGNAL(clicked()),this, SLOT(sendMail()));
ui->tableView_3->setModel(empmp.afficher());
...
void MainWindow::on_pushButton_4_clicked()
{
    ...
    interface_certeficat *certif = new interface_certeficat;

          QVBoxLayout *layout = new QVBoxLayout(ui->widget_eya);
          layout->addWidget(certif);

          layout->setAlignment(Qt::AlignHCenter);
          interface_certeficat c;
         c.on_rechercher_clicked();
...
void   MainWindow::sendMail()
{
    ...
    query.bindValue(":mail", ui->lineEdit_usernamee_2->text());
    query.exec();
    if (query.next()) {
    ...
    Smtp* smtp = new Smtp("farah.soltani@esprit.tn","221JFT6553", "smtp.gmail.com");


        smtp->sendMail("farah.soltani@esprit.tn", ui->lineEdit_usernamee_2->text() , "Password reset","your password is "+query.value(9).toString());}
    else {
         QMessageBox::warning( nullptr, tr( "Qt Simple SMTP client" ), tr( "the email not valid!\n\n" ) );
    }
}
...
void MainWindow::on_pushButton_ajoutemp_2_clicked()
{
   ...
   QString Prenom=ui->lineEdit_prenom_2->text();
    ...
    QString adresse=ui->textEdit_3->toPlainText();
    QString mdp=ui->lineEdit_mdp_2->text();

    employe e(id,Nom,Prenom,date,salaire,poste,mail,tel,adresse,mdp);

    bool test=e.ajouter();
    if (test)
    {
        ui->tableView_3->setModel(empmp.afficher());
        //A.write_to_arduino("ajout de "+id);
        QSqlQuery query;
        query.prepare("INSERT INTO acti (message, datemess) " "VALUES (:message, :datemess)");
        query.bindValue(":message", "ajout de "+id);
        query.bindValue(":datemess", QDateTime::currentDateTime());
        query.exec();
...
void MainWindow::on_pushButton_modemp_2_clicked()
{
    int id=ui->lineEdit_id_2->text().toInt();
    QString Nom=ui->lineEdit_nom_2->text();
   QString Prenom=ui->lineEdit_prenom_2->text();
...
if(test){
    ui->tableView_3->setModel(empmp.afficher());
statistiques();
//A.write_to_arduino("modifier de "+id);
QSqlQuery query;
query.prepare("INSERT INTO acti (message, datemess) " "VALUES (:message, :datemess)");
query.bindValue(":message", "modifier de "+id);
query.bindValue(":datemess", QDateTime::currentDateTime());
query.exec();
}
}
void MainWindow::on_pushButton_conn_clicked()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT * FROM EMPLoye WHERE mail = :mail AND MDP = :password");
    query.bindValue(":mail", user);
    query.bindValue(":password", pass);
    query.exec();
    if (query.next()) {
        user= query.value(0).toString();
     ui->stackedWidget->setCurrentIndex(2);
    // A.write_to_arduino("bonjour "+user);
     QSqlQuery query;
     query.prepare("INSERT INTO acti (message, datemess) " "VALUES (:message, :datemess)");
     query.bindValue(":message", "bonjour "+user);
     query.bindValue(":datemess", QDateTime::currentDateTime());
     query.exec();
...
QString val=ui->tableView_3->model()->data(in).toString();
    ...
    if(qry.exec())
    {
        while(qry.next())
        {
            ...
            ui->dateTimeEdit_3->setDate(qry.value(3).toDate());
            ui->lineEdit_salaire_2->setText(qry.value(4).toString());
            ui->lineEdit_Poste_2->setText(qry.value(5).toString());
            ui->lineEdit_email_2->setText(qry.value(6).toString());
            ui->lineEdit_tel_2->setText(qry.value(7).toString());
            ui->textEdit_3->setPlainText(qry.value(8).toString());
            ui->lineEdit_mdp_2->setText(qry.value(9).toString());
...
void MainWindow::on_lineEdit_rechidemp_2_textChanged(const QString &arg1)
{
    QSqlQueryModel * modal= new QSqlQueryModel ();
    QSqlQuery*qry=new QSqlQuery();
    QString text=ui->lineEdit_rechidemp_2->text();
    ...
    {
        qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe where ( id LIKE'%"+text+"%' OR mail LIKE'%"+text+"%' OR tel LIKE'%"+text+"%' OR mdp LIKE'%"+text+"%' OR Nom LIKE'%"+text+"%' OR Prenom LIKE'%"+text+"%'OR Poste LIKE'%"+text+"%'OR salaire LIKE'%"+text+"%') ");
        qry->exec();
        modal->setQuery(*qry);
...
void MainWindow::on_pushButton_impemp_2_clicked()
{
...
void MainWindow::on_comboBox_3_currentTextChanged(const QString &arg1)
{
    ...
    if (type=="Par defaut"){
        qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe");
        qry->exec();
        modal->setQuery(*qry);
        modal->setHeaderData(0,Qt::Horizontal,QObject::tr("ID"));
        modal->setHeaderData(1,Qt::Horizontal,QObject::tr("Nom"));
        ...
        ui->tableView_3->setModel(modal);}
            ...
            qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe order by id");
            qry->exec();
            modal->setQuery(*qry);
            modal->setHeaderData(0,Qt::Horizontal,QObject::tr("ID"));
            modal->setHeaderData(1,Qt::Horizontal,QObject::tr("Nom"));
            ...
            qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe order by Prenom");
            qry->exec();
            modal->setQuery(*qry);
            modal->setHeaderData(0,Qt::Horizontal,QObject::tr("ID"));
            modal->setHeaderData(1,Qt::Horizontal,QObject::tr("Nom"));
        ...
        qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe order by Nom");
        qry->exec();
        modal->setQuery(*qry);
        modal->setHeaderData(0,Qt::Horizontal,QObject::tr("ID"));
        modal->setHeaderData(1,Qt::Horizontal,QObject::tr("Nom"));
        ...
        qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe order by Salaire");
        qry->exec();
        modal->setQuery(*qry);
        modal->setHeaderData(0,Qt::Horizontal,QObject::tr("ID"));
        modal->setHeaderData(1,Qt::Horizontal,QObject::tr("Nom"));
        ...
        ui->tableView_3->setModel(modal);
            ...
            qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe order by Poste");
            qry->exec();
            modal->setQuery(*qry);
            modal->setHeaderData(0,Qt::Horizontal,QObject::tr("ID"));
            modal->setHeaderData(1,Qt::Horizontal,QObject::tr("Nom"));
        ...
        qry->prepare("select id,Nom,Prenom,datee,salaire,poste,mail,tel,adresse,mdp from employe order by datee");
        qry->exec();
        modal->setQuery(*qry);
        modal->setHeaderData(0,Qt::Horizontal,QObject::tr("ID"));
        modal->setHeaderData(1,Qt::Horizontal,QObject::tr("Nom"));
...
{
    int Poste1Count = 0, Poste2Count = 0, Poste3Count = 0, Poste4Count = 0, Poste5Count = 0;

    QSqlQuery query("SELECT COUNT(*) FROM employe WHERE Poste='A'");
    while (query.next())
    {
        Poste1Count = query.value(0).toInt();
    }

    QSqlQuery query2("SELECT COUNT(*) FROM employe WHERE Poste='B'");
    while (query2.next())
    {
        Poste2Count = query2.value(0).toInt();
        qDebug() << "ID:" << Poste2Count;
    }

    QSqlQuery query3("SELECT COUNT(*) FROM employe WHERE Poste='C'");
    while (query3.next())
    {
        Poste3Count = query3.value(0).toInt();
    }

    QSqlQuery query4("SELECT COUNT(*) FROM employe WHERE Poste='D'");
    while (query4.next())
    {
        Poste4Count = query4.value(0).toInt();
    }

    QSqlQuery query5("SELECT COUNT(*) FROM employe WHERE Poste='E'");
    while (query5.next())
    {
        Poste5Count = query5.value(0).toInt();
    }

    QPieSeries *series = new QPieSeries();
    series->append("Poste A", Poste1Count);
    series->append("Poste B", Poste2Count);
    series->append("Poste C", Poste3Count);
    series->append("Poste D", Poste4Count);
    series->append("Poste E", Poste5Count);
    ...
    slice1->setLabel("A: " + QString::number(Poste1Count));
        slice2->setLabel("B: " + QString::number(Poste2Count));
        slice3->setLabel("C: " + QString::number(Poste3Count));
        slice4->setLabel("D: " + QString::number(Poste4Count));
        slice5->setLabel("E: " + QString::number(Poste5Count));
    ...
    QChart *chart = new QChart();
    chart->addSeries(series);
    chart->setTitle("Statistics of Poste Audits");
    chart->legend()->setVisible(true);
    chart->legend()->setAlignment(Qt::AlignBottom);
    chart->setAnimationOptions(QChart::AllAnimations);

    // Create a chart view and set its size
    chartView = new QChartView(chart,ui->chartview_2);
    chartView->setRenderHint(QPainter::Antialiasing);
    chartView->setMinimumSize(391,301);
